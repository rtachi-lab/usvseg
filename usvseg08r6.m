function varargout = usvseg08r6(action)
% usvseg08r6
%
...
% Initiation ------------------------------------------------------
if nargin==0
    gh.version = 'USVSEG ver 0.8 (rev 6)';
...
if strcmp(action,'long')
    ...
    for r=1:nread
        ...
        mtsimrng = median(mtsp(:)) + [0 40];
        fltndimrng = [0 30];
        set(gh.htext_fnam,'string',['File: ' fp ' ...  (' num2str(r) '/' num2str(nread) ')']);
        % save to global 
...
if strcmp(action,'fldr')
    ...
    hwf = waitbar(0,'');
    for f=1:nfile
        waitbar(f/nfile,hwf,sprintf('(%d/%d files) %s',f,nfile,fns{f})); 
        % get wavfile
...
set(hfig,'KeyPressFcn','usvseg08r6(get(gcf,''CurrentCharacter''));');
set(hfig,'DeleteFcn',@finishfunc);
% axes
haxes(1) = axes; axis off
haxes(2) = axes; axis off
haxes(3) = axes; axis off
% uicontrol
...
% callbacks
set(hpush_open,'callback','usvseg08r6(''open'');');
set(hpush_flat,'callback','usvseg08r6(''flat'');');
set(hpush_thrs,'callback','usvseg08r6(''thrs'');');
set(hpush_dtct,'callback','usvseg08r6(''dtct'');');
set(hpush_save,'callback','usvseg08r6(''save'');');
set(hpush_sgmt,'callback','usvseg08r6(''sgmt'');');
set(hpush_play,'callback','usvseg08r6(''play'');');
set(hpush_swav,'callback','usvseg08r6(''swav'');');
set(hpush_long,'callback','usvseg08r6(''long'');');
set(hpush_fldr,'callback','usvseg08r6(''fldr'');');
% text set
...
% filename and axis
set(htext_fnam,'units','normalized','position',[0.16 0.96 0.77 0.02]);
set(haxes(1), 'units','normalized','position',[0.20 0.70 0.77 0.25]);
set(haxes(2), 'units','normalized','position',[0.20 0.40 0.77 0.25]);
set(haxes(3), 'units','normalized','position',[0.20 0.10 0.77 0.25]);
% disable
...
hwp = waitbar(0/4,'Process...','position',[10,40,270,50]);
% multitaper spec
mtsp = multitaperspec(wav,fs,fftsize,timestep);
% waitbar
waitbar(1/4,hwp);
...
hw = waitbar(0.7,'Read...','position',[10,40,270,50]); 
